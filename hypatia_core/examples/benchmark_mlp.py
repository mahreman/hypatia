...
try:
    ...
    # print("⚠️  transformers bulunamadı. Phi-3 testi devre dışı.") # MLP testinde gerek yok
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # ✅ DÜZELTME: FX Trace'in "size" metodunu görmesi yerine
        # 'torch.flatten' fonksiyonunu görmesi için güncellendi.
...
def get_model_and_input_shape(model_name: str) -> Tuple[nn.Module, Tuple[int, ...]]:
    ...
    # (Diğer modeller bu betikten kaldırıldı)
    else:
...
def get_dummy_inputs(
    ...
    if len(input_shape) == 1:
        # Transformer (Bu betikte yok ama uyumluluk için duruyor)
...
def optimize_model_from_base(
    ...
    try:
        ...
        # ====================================================================
        # ✅ ADIM 1: GÜNCELLENMİŞ TRACE ÇAĞRISI
        ...
        # ====================================================================
        # ✅ ADIM 2: MODÜL TİP HARİTASI OLUŞTURMA
        # Rust'a modül adlarının (örn: 'layers.0') tiplerini (örn: 'Linear') bildir.
        # ====================================================================
        module_type_map = {}
        for node in graph_module.graph.nodes:
            if node.op == 'call_module':
                # node.target = 'layers.0', 'layers.1' vb.
                module = graph_module.get_submodule(node.target)
                module_type_map[node.target] = type(module).__name__
                # Örn: module_type_map['layers.0'] = 'Linear'
        # ====================================================================
        ...
        if HYPATIA_AVAILABLE and hasattr(hypatia_core, "compile_fx_graph"):
            ...
            # ====================================================================
            # ✅ ADIM 3: GÜNCELLENMİŞ RUST ÇAĞRISI
            # Haritayı Rust fonksiyonuna üçüncü bir argüman olarak gönder.
            # ====================================================================
...
def main():
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default=f"results/benchmark_mlp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    ...
    if all_results:
        ...
        print(f"\n✅ MLP benchmark sonuçları şuraya yazıldı: {args.output_csv}")
        
        print("\n" + "="*100)
        print("DETAYLI MLP BENCHMARK ÖZETİ")
        print("="*100)
        cols_to_show = [
            "scenario", "precision", 
