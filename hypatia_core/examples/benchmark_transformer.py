#!/usr/bin/env python3
"""
Tiny-Transformer Benchmark Script (Hypatia)

Senaryolar:
- Transformer-Small  (batch=32)
- Transformer-Large  (batch=128)

FP32, FP16, BF16 hassasiyetlerinde:
- Baseline
- Hypatia (compile_fx_graph)
"""
...
def main():
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default=f"results/benchmark_transformer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    ...
    device = get_device()
    print("=" * 80)
    print("HYPATIA TINY-TRANSFORMER BENCHMARK")
    print("=" * 80)
        ...
        for scenario_name, (batch_size, model_name) in SCENARIOS.items():
            ...
            try:
                ...
                res_base = run_benchmark(
                    ...
                    precision=precision,
                    precision_str=precision_str, # <-- ✅ GÜNCELLENDİ
                    original_model=None,
    ...
    if all_results:
        ...
        print(f"\n✅ Tiny-Transformer benchmark sonuçları şuraya yazıldı: {args.output_csv}")

        print("\n" + "=" * 100)
        print("DETAYLI TRANSFORMER BENCHMARK ÖZETİ")
        print("=" * 100)
        cols_to_show = [
            "scenario",
            "precision",
